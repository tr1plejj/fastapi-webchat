...
async function loadMessages(userId) {
    try {
        const response = await fetch(`http://127.0.0.1:8000/chat/messages/${userId}/`);
...
function connectWebSocket() {
    ...
    socket = new WebSocket(`ws://127.0.0.1:8000/chat/ws/${selectedUserId}/`);
...
function createMessageElement(text, recipient_id) {
    ...
    const messageClass = userID === recipient_id ? 'sent message-content' : 'received message-content';
